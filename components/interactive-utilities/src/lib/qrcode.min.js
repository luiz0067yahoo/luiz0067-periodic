/**
 * QRCode.js - Minimal self-contained QR Code generator
 * Compatible with browsers, Node, and WordPress Gutenberg
 */
(function(root, factory) {
      ...
      if (this.typeNumber < 1) {
        ...
        for (; typeNumber < 40; typeNumber++) {
          ...
          for (var i = 0; i < this.dataList.length; i++) {
            ...
            buffer.put(data.getLength(), QRUtil.getLengthInBits ? QRUtil.getLengthInBits(data.mode, typeNumber) : 8);
            data.write(buffer);
          }
    ...
    if (this._el) {
      ...
      for (var r = 0; r < count; r++) {
        for (var c = 0; c < count; c++) {
          if (qr.isDark(r, c)) {
            var x = Math.round(c * cellWidth);
            var y = Math.round(r * cellHeight);
            var w = Math.ceil((c + 1) * cellWidth) - x;
            var h = Math.ceil((r + 1) * cellHeight) - y;
  ...
  QRCode.generateSVG = function(text, options) {
    options = options || {};
    var colorDark = options.colorDark || options.color || "#000000";
    var colorLight = options.colorLight || options.bgColor || "#ffffff";
    var width = options.width || 256;
    var height = options.height || 256;

    var qr = new QRCodeModel(0, 2);
    qr.addData(text || "");
    qr.make();
    ...
    for (var r = 0; r < count; r++) {
      for (var c = 0; c < count; c++) {
        if (qr.isDark(r, c)) {
          var x = (c * cell).toFixed(2);
          var y = (r * cell).toFixed(2);
          var w = (cell + 0.05).toFixed(2);
          var h = (cell + 0.05).toFixed(2);
